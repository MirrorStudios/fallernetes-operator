...
var basicServerSpec = gameserverv1alpha1.ServerSpec{
	Pod: corev1.PodSpec{
...
func (p TestChecker) IsDeletionAllowed(server *gameserverv1alpha1.Server, pod *corev1.Pod) (bool, error) {
	...
	Context("Reconcile logic", func() {
		...
		It("should create a Pod for the Server", func() {
			...
			for _, event := range recorder.Events {
				...
				_, serverOk := event.Object.(*gameserverv1alpha1.Server)
		...
		It("should handle deletion of the Server and remove the Pod", func() {
			...
			for _, event := range fakeRecorder.Events {
				...
				_, isServer := event.Object.(*gameserverv1alpha1.Server)
